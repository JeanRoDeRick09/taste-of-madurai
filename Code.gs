...
const PRICES = { mutton: 300, chicken: 250 };
...
function getSheet_() {
  ...
  if (!sheet) {
    ...
    sheet.appendRow([
      ...
      "Mutton Qty", "Chicken Qty", "Amount", "Screenshot URL", "Paid"
    ]);
  }
  return sheet;
}
...
function doPost(e) {
  ...
  const amount = mutton * PRICES.mutton + chicken * PRICES.chicken;
  ...
  if (body.screenshot) {
    try {
      ...
      screenshotUrl = "https://drive.google.com/uc?id=" + file.getId();
    } catch (err) {
      screenshotUrl = "";
    }
  }
